...
export const toolUtil = {
  ...
  merge(target: any, source: any, overwrite: boolean) {
    const self = this;
  ...
  formatCharts: function formatCharts(value: String, length: number) {
    ...
    for (let i = 0, l = value.length; i < l; i++) {
      ...
      if (/.*[\u4e00-\u9fa5]+.*$/.test(value[i])) {
        ...
        if (i < l - 1 && objLength % length === 0 && i !== 0) {
          ...
          i < l - 1 &&
          Math.floor(objLength / length) > lines &&
          i !== 0
        ) {
          res = `${res}\n${value[i]}`; // 就是这里！！！
          lines++;
  ...
  /*
    ...
    */
  dataUnit(type: String, unit: String, value: any, max: number = 0, digit?: number, retainedDecimal?: Boolean) {
  ...
  /*
     ...
     @author hy
     */
  iotDateFormat(v?: any, tp?: string, op?: any) {
    if (!v) {
      return false;
    }
    v = v.replace(/-/g, '');
  ...
  isBuiltInObject(value: any) {
    ...
    const BUILTIN_OBJECT: { [key: string]: number | string } = {
      '[object Function]': 1,
      '[object RegExp]': 1,
      '[object Date]': 1,
      '[object Error]': 1,
  ...
  clone(source: any) {
    ...
    const TYPED_ARRAY: {[key: string]: number | string} = {
      '[object Int8Array]': 1,
      '[object Uint8Array]': 1,
